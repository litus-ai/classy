...
from ...data.data_drivers import GENERATION, QA, SENTENCE_PAIR, SEQUENCE, TOKEN
from ...utils.help_cli import HELP_TASKS
from ...utils.hydra_patch import ConfigBlame
from ...utils.log import get_project_logger
from .utils import get_device, maybe_find_directory
...
def _main_mock(cfg, blames: Optional[List] = None):
    ...
    if dry_run_for_print:
        from ...utils.rich_config import print_config
...
def _main_resume(model_dir: str):
    ...
    from ...utils.io import load_training_conf

    model_dir = Path(model_dir)

    last_ckpt_path = model_dir / "checkpoints/last.ckpt"
    cfg = load_training_conf(str(last_ckpt_path))
...
def classy_merge(
    ...
    from ...utils.omegaconf import is_interpolation
