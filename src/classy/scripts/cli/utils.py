...
from ...utils.experiment import Experiment, Run
from ...utils.file import CLASSY_MODELS_CACHE_PATH
...
def autocomplete_model_path(prefix: str, include_dry_model: bool = False, **kwargs):
    def f():
        ...
        if "/" in prefix:
            ...
            candidates = list(
                Experiment.from_name(exp_name).iter_candidate_runs(prefix)
            )
            # warn(candidates)
            if len(candidates) > 1:
    ...
    possible_model_paths = f()
    if include_dry_model:
        possible_model_paths += [DRY_MODEL]

    return possible_model_paths


def checkpoint_path_from_user_input(
    model_path: str, include_dry_model: bool = False
) -> str:
    if include_dry_model and model_path == DRY_MODEL:
        return model_path
