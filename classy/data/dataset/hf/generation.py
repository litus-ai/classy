    ...
    def sample2elem_dict(self, sample: GenerationSample, inference_mode: bool) -> Dict[str, Any]:
        ...
        if not inference_mode:
            ...
            elem_dict["decoder_start"] = torch.tensor(
                [self.tokenizer.lang_code_to_id[self.mbart_l2l_code[sample.target_language]]]
            )
        ...
        if not inference_mode:
            if sample.target_sequence is not None:
                ...
                elem_dict["input_ids"] = torch.cat(
                    (elem_dict["input_ids"], tokenization_output["input_ids"].squeeze()),
                    dim=0,
                )
                elem_dict["attention_mask"] = torch.cat(
                    (elem_dict["attention_mask"], tokenization_output["attention_mask"].squeeze()),
                    dim=0,
                )
            else:
                # assume standard clm
